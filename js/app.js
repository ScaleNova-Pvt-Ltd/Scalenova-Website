/**
 * ScaleNova Main Application Bootstrap (Refined Landing Page & Deep-Content Architecture)
 * 
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 3. Render 6 Concise Feature Cards on Homepage
  ...
  if (featuresContainer && typeof SCALENOVA_FEATURES !== 'undefined') {
    featuresContainer.innerHTML = SCALENOVA_FEATURES.map(f => `
      <div class="sn-card sn-card-hover p-5 sm:p-6 flex flex-col justify-between group cursor-pointer" onclick="ScaleNovaModals.openFeatureDetail('${f.id}')" role="button" tabindex="0" onkeydown="if(event.key==='Enter')ScaleNovaModals.openFeatureDetail('${f.id}')" aria-label="Explore ${f.title} Details">
        <div class="space-y-3">
          ...
          <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-medium leading-relaxed">
            ${f.shortSummary || f.summary4Lines[0]}
          </p>
        </div>

        <div class="pt-3.5 mt-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-xs font-bold text-brand-600 dark:text-brand-400">
  ...
  if (blogContainer && typeof SCALENOVA_BLOGS !== 'undefined') {
    // Show first 3 articles on landing page
